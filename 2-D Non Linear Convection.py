...
for n in range (nt + 1):
    ...
    vn = v.copy()
    u[1:,1:]=(un[1:,1:]-
              (un[1:,1:] * c * dt / dx * (un [1:,1:] - un[1:, 0:-1])) -
              (vn[1:,1:] * c * dt / dy * (un [1:,1:] - un[0:-1, 1:])))

    v[1:,1:]=(vn[1:,1:]-
              (vn[1:,1:] * c * dt / dx * (vn [1:,1:] - vn[1:, 0:-1])) -
              (un[1:,1:] * c * dt / dy * (vn [1:,1:] - vn[0:-1, 1:])))
    u[ 0 , : ] = 1
    u[ -1, : ] = 1
    u[ : , 0 ] = 1
    u[: , -1 ] = 1
    ...
    pyplot.savefig("nonlinearconvec" +  str(n) + ".png", format="PNG")
